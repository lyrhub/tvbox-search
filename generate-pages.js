...
function generateStatusPage() {
  ...
  const siteRows = sorted.map(([key, r]) => {
    ...
    let noteHtml = '';
    if (r.status === 'ok') noteHtml = `<span style="color:#3fb950">✓ 可搜索</span>`;
    else if (r.status === 'skip') noteHtml = `<span class="skip-text">${r.reason === 'no_host' ? '无法获取地址' : r.reason}</span>`;
    else {
      const reasonMap = {
        'all_paths_failed': '所有搜索路径失败',
        'timeout': '超时',
      ...
      noteHtml = `<span class="error-text">${reasonMap[r.reason] || r.reason || '失败'}</span>`;
    }
...
.api-url{font-family:"SF Mono",Monaco,monospace;font-size:12px;color:#8b949e;max-width:350px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
<h1>🔍 TVBox Search</h1>
<p class="subtitle">定时检测 TVBox 源站点搜索功能，只保留支持搜索的站点</p>
<div class="nav">
  <a href="./searchable.json">📋 可搜索配置 JSON</a>
  <a href="./results.json">📊 测试结果 JSON</a>
</div>
<div class="info-bar">⏱ 最后测试: ${new Date(testedAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })} | 测试关键词: "${keyword}" | 每30分钟自动运行</div>
<div class="summary">
  <div class="stat stat-total"><div class="stat-value">${summary.total || totalTested}</div><div class="stat-label">总站点</div></div>
  <div class="stat stat-ok"><div class="stat-value">${okCount}</div><div class="stat-label">可搜索</div></div>
  <div class="stat stat-fail"><div class="stat-value">${failCount}</div><div class="stat-label">搜索失败</div></div>
  <div class="stat stat-skip"><div class="stat-value">${skipCount}</div><div class="stat-label">跳过</div></div>
</div>
<div class="summary">
  <div class="stat stat-ok"><div class="stat-value">${outputSites}</div><div class="stat-label">输出站点</div></div>
  <div class="stat"><div class="stat-value" style="color:#d29922">${summary.excluded || 0}</div><div class="stat-label">排除(网盘等)</div></div>
</div>
...
<h3 class="section-title">🔍 站点搜索测试详情</h3>
<table><thead><tr><th>状态</th><th>站点</th><th>类型</th><th>搜索地址</th><th>延迟</th><th>结果</th></tr></thead><tbody>${siteRows}</tbody></table>

<div class="footer">TVBox Search | GitHub Pages | 自动更新 | 测试关键词: ${keyword}</div>
</div></body></html>`;
}
